    ...
    {adjustments.map((adjustment) => {
      ...
      if (!isEditing) return <div key={adjustment.id} className="flex items-center justify-between gap-3 rounded-lg border bg-background p-3">
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-muted-foreground">{adjustment.workDate ? `${adjustment.workDate} · ` : ""}{adjustment.description}</p>
        </div>
      ...
      return <div key={adjustment.id} className="rounded-lg border bg-background p-3">
        <form action={updateTimeAdjustment} className="grid gap-3">
          ...
          <label className="grid gap-1 text-sm"><span>Data do acerto</span><input name="workDate" type="date" defaultValue={adjustment.workDate ?? ""} className="h-9 rounded-md border bg-background px-3" /></label><label className="grid gap-1 text-sm"><span>Descrição / motivo</span><input name="description" required defaultValue={adjustment.description} className="h-9 rounded-md border bg-background px-3" /></label>
