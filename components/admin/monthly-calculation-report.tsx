...
export function MonthlyCalculationReport({ entries, adjustments, member, year, month }: { entries: TimeEntry[]; adjustments: TimeAdjustment[]; member: Staff; year: number; month: number }) {
  ...
  const rows = Array.from({ length: daysInMonth }, (_, index) => {
    ...
    return { date, entry, calc: entry ? calculateDay(entry, member) : null, dailyDebits }
  })
  ...
  return <section className="monthly-calculation-report" aria-label="Memória de cálculo mensal">
    ...
    <table className="monthly-report-table"><thead><tr><th>Data</th><th>Marcações</th><th>Jornada prevista</th><th>Trabalhado</th><th>Crédito</th><th>Débito</th><th>Tratamento / observação</th></tr></thead><tbody>{rows.map(({ date, entry, calc, dailyDebits }) => { const balance = calc?.balanceMinutes ?? 0; const treatmentLabels: Record<string, string> = { normal: "Registro normal", unjustified_absence: "Falta injustificada", justified_absence: "Falta justificada", medical_certificate: "Atestado médico", compensatory_leave: "Folga compensatória", early_departure: "Saída antecipada", compensatory_early_departure: "Saída antecipada compensatória", holiday: "Feriado", day_off: "Folga" }; const observation = !entry ? "Sem registro" : calc?.alertas.length ? calc.alertas.map((alerta) => alerta === "INTERVALO_SUPERIOR_AO_PREVISTO" ? "Intervalo superior ao previsto" : alerta === "ALERTA_DE_INTERVALO_IRREGULAR" ? "Intervalo irregular" : alerta === "SABADO_JORNADA_NORMAL_4H" ? "Sábado: jornada normal considerada 09h–13h (4h); excedente lançado no banco" : alerta === "ENTRADA_COMPENSADA" ? "Entrada compensada: sem débito diário" : alerta).join("; ") : treatmentLabels[entry.occurrenceType] ?? "Registro normal"; return <tr key={date}><td>{formatDateBR(date)}</td><td>{entry ? `${formatTime(entry.clockIn)} · ${formatTime(entry.lunchStart)}–${formatTime(entry.lunchEnd)} · ${formatTime(entry.clockOut)}` : "—"}</td><td>{calc ? formatMinutes(calc.scheduledMinutes) : "—"}</td><td>{calc?.complete ? formatMinutes(calc.workedMinutes) : "—"}</td><td>{balance > 0 ? formatMinutes(balance) : "—"}</td><td>{balance < 0 ? `-${formatMinutes(Math.max(0, Math.abs(balance) - dailyDebits))}` : "—"}</td><td>{observation}{dailyDebits ? ` · ${formatMinutes(dailyDebits)} utilizada do banco para compensar o débito do dia` : ""}{calc?.intervalo.excedente ? ` · excedente intervalo: ${calc.intervalo.excedente}min` : ""}</td></tr> })}</tbody></table>
