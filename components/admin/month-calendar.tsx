...
export function MonthCalendar({ entries, adjustments, member, year, month }: { entries: TimeEntry[]; adjustments: TimeAdjustment[]; member: Staff; year: number; month: number }) {
  const [selected, setSelected] = useState<{ date: string; entry: TimeEntry | null } | null>(null)
  // `month` é 1-based para a interface; Date.UTC usa 0-based.
  // O deslocamento garante que o dia 1 caia na coluna real do calendário.
  ...
  return <>
    <div className="grid grid-cols-7 gap-1 text-center text-xs">
      ...
      {Array.from({ length: firstWeekday }, (_, index) => <div key={`empty-${index}`} aria-hidden="true" className="min-h-14 rounded-md border border-transparent" />)}
    ...
    {selected && <EditEntryDialog employeeUserId={member.userId} workDate={selected.date} entry={selected.entry} adjustments={adjustments.filter((adjustment) => adjustment.workDate === selected.date)} open onOpenChange={(open) => !open && setSelected(null)} />}
  </>
}
