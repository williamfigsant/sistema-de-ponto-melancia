...
import { reviewTimeOffRequestWithState, updateTimeOffRequestWithState } from "@/app/actions/time-off"
...
export function ReviewTimeOffForm({ requestId, status }: { requestId: string; status: "approved" | "rejected" }) {
  ...
  useEffect(() => {
    if (state.success) toast.success(status === "approved" ? "Solicitação aprovada e banco de horas atualizado." : "Solicitação rejeitada.")
...
export function UpdateTimeOffForm({ request }: { request: { id: string; workDate: string; status: string; requestType: string; minutes: number; reason: string } }) {
  ...
  useEffect(() => {
    if (state.success) toast.success("Solicitação atualizada e banco de horas sincronizado.")
    if (state.error) toast.error(state.error)
  }, [state])
